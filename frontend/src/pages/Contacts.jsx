...
const Contacts = () => {
  ...
  const handleSendOTP = async () => {
    ...
    try {
      const res = await fetch(
        "https://smart-sos-platform.onrender.com/api/auth/send-otp",
        {
          method: "POST",
      ...
      if (data.success) {
        ...
        const message =
          "🔐 SmartSOS Contact Verification OTP: *" +
          data.otp +
          "*\n\n" +
        ...
        toast.success(
          "📲 WhatsApp opened! Send the message to yourself and note the OTP!",
          {
            autoClose: 6000,
          },
        );
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      const res = await fetch(
        "https://smart-sos-platform.onrender.com/api/auth/verify-otp",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const handleSubmit = async (e) => {
    ...
    if (!phoneVerified) {
      toast.warning(
        "⚠️ Please verify the phone number via WhatsApp OTP first!",
      );
      return;
    }
    ...
    <div style={styles.container}>
          ...
          <form onSubmit={handleSubmit} style={styles.form}>
            ...
            {otpStep && !phoneVerified && (
              <div style={styles.otpBox}>
                <div style={styles.otpHeader}>
                  ...
                  <div>
                    ...
                    <p style={styles.otpSubtitle}>
                      Open WhatsApp → Tap Send on the message → Check your sent
                      messages for the OTP → Enter it below
                    </p>
                  </div>
                </div>

                <div style={styles.otpInputRow}>
                  {/* In the OTP verification section */}
                  <input
                    type="number"
                    placeholder="6-digit OTP" // shorter placeholder
                    value={otpInput}
                    onChange={(e) => setOtpInput(e.target.value)}
                    style={{
                      ...styles.otpInput,
                      boxSizing: "border-box", // ensure padding fits
                    }}
                    maxLength={6}
                  />
                  <button
                ...
                <p style={styles.otpHint}>
                  💡 Tap Send in WhatsApp first, then check sent messages for
                  the OTP number
                </p>
            ...
            {!phoneVerified && form.phone.length >= 10 && (
              <div style={styles.warningBox}>
                ⚠️ Please verify the phone number via WhatsApp OTP before adding
                contact!
              </div>
            )}
          ...
          {contacts.map((c) => (
            <div key={c._id} style={styles.contactCard}>
              ...
              <div style={styles.contactBtns}>
                <a href={"tel:" + c.phone} style={styles.callBtn}>
                  📞 Call
                </a>
                <button
      ...
      <div style={styles.infoBox}>
        ...
        <p style={styles.infoText}>
          All contacts are WhatsApp OTP verified. They will receive your live
          location when you press SOS. Add at least 3 trusted contacts for best
          safety coverage.
        </p>
      </div>
    </div>
